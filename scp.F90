...
!//PARA
  ...
  if (rank == 0) then
    ...
    do ifile = 1, nfile
      ...
      info = 0
      call read_xsf(fname,ngrid,origin,span,val0,stepvec,info)
      ...
      call ferdi(l,m,val0,ngrid,origin,stepvec,Rnl)
      val = real(Rnl)
      deallocate(Rnl)
      ...
      if (thr < (1.0 - EPS9)) then
        ...
        call isovalue_scale(thrpow,thr,isovalue(ifile),val0,ngrid)
        call scalar_trunc(isovalue(ifile),ngrid,val0,stepvec,&
